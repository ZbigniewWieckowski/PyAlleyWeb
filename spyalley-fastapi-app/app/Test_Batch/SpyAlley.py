...
class SpyAlleyGame:
    ...
    def getActivePlayers(self):
        return self.ActivePlayers
    def play(self, bot_class, bot_name):
        ...
        for pn in range(0,6-self.humanCount):
            ...
            if len(availableNationalities) == 1:
                ...
                nat = availableNationalities[pos-1]
            if pn % 2 == 0:
                player = bot_class(bot_name +' #' + str(math.ceil((pn + 1) / 2)), nat, self.suppressPrint)
            else:
                player = Z00_Bot('Z00_Bot #' + str(math.ceil((pn + 1) / 2)), nat, self.suppressPrint)
            self.board.addPlayer(player)
    ...
    def botMatchup(self, bot_class, bot_name):
        stats = { }
        for i in range(0,1000):
            # print(str(i))
            self.play(bot_class, bot_name)
            win = self.getBoard().getActivePlayers()[0].getName()
            if win in stats:
                stats[win] += 1
            else:
                stats[win] = 1
